...
% Regra para pesquisar e listar as capitais dos estados de uma determinada região
pesquisar_capitais_por_regiao :-
    write('Digite o nome da regiao: '),
    read(Regiao),
    nl,
    write('As capitais dos estados da regiao '), write(Regiao), write(' sao:'), nl,
    regiao(Sigla, Regiao),
    capital(Capital, Sigla),
    write(Capital), nl,
    fail.

pesquisar_capitais_por_regiao :- nl.
